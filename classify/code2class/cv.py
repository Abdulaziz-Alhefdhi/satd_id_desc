...
trained_models_dir = "/home/aziz/experiments/trained_models/td/classify/CT/cv/"
...
input_token_index, reverse_input_token_index = token_integer_mapping(all_input_vocab)
...
for train_index, test_index in skf.split(cv_set.input_lists, cv_set.labels):
    ...
    model = build_model(latent_dim, len(train_vocab), num_layers)
    model.summary()
    ...
    for i in range(1, epochs+1):
        ...
        model.fit(train_model_inputs, train_model_outputs, batch_size=batch_size, callbacks=[checkpoint])  # Train
